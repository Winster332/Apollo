...
export class Store {
	constructor(props: ICallCenterConstructorAppSettings) {
		makeObservable(this)
		console.log(props)

		this.workspace = new Workspace(this.openEditor);

		this.workspace.addBegin(100, 100)
	...
	public init = () => {
		...
		this.historyHeight = phoneCallElement === null
			? 0
			: window.innerHeight - 59 - parseFloat((window.getComputedStyle(phoneCallElement)?.height || '0').replace('px', ''))
	};
